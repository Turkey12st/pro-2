-- إصلاح مشاكل الأمان وإضافة تحسينات أساسية (مُصحح)
...
-- إنشاء دالة محسنة للأذونات (مُصححة)
...
AS $$
DECLARE
  user_role_exists BOOLEAN := FALSE;
...
BEGIN
  ...
  -- التحقق من الأذونات في الأدوار
  SELECT EXISTS (
    SELECT 1 FROM public.user_roles ur
    WHERE ur.user_id = _user_id
    AND ur.permissions IS NOT NULL
    AND ur.permissions ? _permission
